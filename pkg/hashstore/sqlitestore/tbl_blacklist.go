...
func (w *blacklist) FindByMD5(md5 string) (*hashstore.BlacklistRow, error) {
	row := &hashstore.BlacklistRow{}
...
func (b *blacklist) Insert(row hashstore.BlacklistRow) error {
...
func (b *blacklist) InsertBatch(rows []hashstore.BlacklistRow) error {
	b.prepareStmtOnce.Do(func() {
		b.prepareBulkInsertStmt(BlacklistBatchSize)
		b.prepareInsertStmt()
	})

	insertBatch := func(batch []hashstore.BlacklistRow, stmt *sqlx.Stmt) {
